...
const UI = {
  ...
  toast(msg, type = 'ok', duration = 3200) {
    ...
    el.innerHTML = `<span style="flex-shrink:0;font-size:15px">${icons[type]||'•'}</span><span>${this.esc(msg)}</span>`;
    c.appendChild(el);
    // Auto remove
    const remove = () => {
      el.style.opacity   = '0';
    ...
    el._removeTimer = setTimeout(remove, duration);
    el.addEventListener('click', () => { clearTimeout(el._removeTimer); remove(); });
  },
