...
const Report = {
  ...
  _kpis(r) {
    const quebraInfo = (r.quebraPC > 0)
      ? `<div class="kpi"><div class="kpi-val">${r.quebraPC}%</div><div class="kpi-lbl">Quebra Aplicada</div></div>
         <div class="kpi"><div class="kpi-val">${UI.fmtM(r.mComQuebra)}</div><div class="kpi-lbl">Metros c/ Quebra</div></div>`
      : '';
    document.getElementById('rpt-kpis').innerHTML = `
      ...
      <div class="kpi">
        <div class="kpi-val">${UI.fmtM(r.mPedido ?? r.mTotal)}</div>
  ...
  _details(r) {
    ...
    let html = `
      ...
      <div class="cb-row"><span class="cb-k">Metragem pedido</span><span class="cb-v">${UI.fmtM(r.mPedido ?? r.mTotal)} m</span></div>
      <div class="cb-row"><span class="cb-k">Quebra aplicada</span><span class="cb-v">${r.quebraPC > 0 ? r.quebraPC + '%  →  ' + UI.fmtM(r.mComQuebra) + ' m' : '0% (sem quebra)'}</span></div>
